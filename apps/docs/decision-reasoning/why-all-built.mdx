...
<Warning>
	It is not uncommon to see the TS LSP taking 1.5-3 seconds for every autocompletion
	suggestion to come up, the same goes for error reporting. This absolutely kills your
	productivity, and switching to building those packages immediately fixes it.
</Warning>
...
```bash
bun turbo dev --filter=@monorepo/web... --filter=!./packages/config/* --filter=!@monorepo/ui --only
```
...
{
	"scripts": {
		"dev:web:api": "bun turbo dev --filter=@monorepo/web... --filter=!./packages/config/* --filter=!@monorepo/ui --only"
	}
}
```
