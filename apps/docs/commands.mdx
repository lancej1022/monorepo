...
<Note>
	monorepo uses Turborepo to run commands across apps & packages in the monorepo, you can
	also use `bun --filter` commands to do the same without dependant tasks running or
	caching.
</Note>
...
You can start up for example only the web application by using `bun turbo dev --filter=@monorepo/web`.
...
You can build for example only the web application by using `bun turbo build --filter=@monorepo/web`.
...
monorepo supports all of [DrizzleKit's commands](https://orm.drizzle.team/kit-docs/commands) which are all configured for you in the `packages/db/package.json` file.

You can invoke these commands using `bun`, for example to check for collisions and inconsistencies between your database and the schema, you can run `bun --filter @monorepo/db db:check`.
